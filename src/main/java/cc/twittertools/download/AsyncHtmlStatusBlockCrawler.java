...
public class AsyncHtmlStatusBlockCrawler {
  ...
  private class TweetFetcherHandler extends AsyncCompletionHandler<Response> {
    ...
    // TODO make these pattern.compile(foo)
    protected Pattern data_tweet_id = "data-tweet-id=\"(\\d+)\"";
    protected Pattern data_screen_name = "data-screen-name=\"([A-Za-z0-9_]+)\"";
    protected Pattern data_user_id = "data-user-id=\"(\\d+)\"";
    protected Pattern data_mentions = "data-mentions=\"([A-Za-z0-9_ ]+)\"";
    // data_time is an int which needs translating
    protected Pattern data_time = "data-time=\"(\\d+)\""; 
    // tweet text is HTML
    protected Pattern js_tweet_text = "<p class\"js-tweet-text\">(.+?)</p>";
    protected Pattern retweet_count = "Retweeted (\\d+) times";
    protected Pattern fave_count = "Favorited (\\d+) times";
    protected Pattern data_placename = "data-placename=\"(.+?)\">";
    // in-reply-tos are going to be complicated
    // user data is going to require an extra fetch
